 ...
 * return.
 */
public class CoinReceptacleSimulator extends
		AbstractHardware<CoinReceptacleListener> implements
	...
	private HashMap<Integer, CoinChannelSimulator> coinRacks = new HashMap<Integer, CoinChannelSimulator>();
	...
	public CoinReceptacleSimulator(int capacity) {
		if (capacity <= 0)
			throw new SimulationException("Capacity must be positive: "
					+ capacity);
	...
	public void loadWithoutEvents(Coin... coins) throws SimulationException {
		if (maxCapacity < coinsEntered.size() + coins.length)
			throw new SimulationException(
					"Capacity exceeded by attempt to load");
	 ...
	 *             if the receptacle is disabled.
	 */
	public void storeCoins() throws CapacityExceededException,
			DisabledException {
		if (isDisabled())
			throw new DisabledException();

		for (Coin coin : coinsEntered) {
			CoinChannelSimulator ccs = coinRacks.get(new Integer(coin
					.getValue()));
			...
			if (ccs != null && ccs.hasSpace())
				...
				throw new SimulationException(
						"The 'other' output channel has not been defined, but it is needed for storage.");
		}
	 ...
	 *             if the receptacle is disabled.
	 */
	public void returnCoins() throws CapacityExceededException,
			DisabledException {
		if (isDisabled())
			throw new DisabledException();
	...
	private void notifyCoinAdded(Coin coin) {
		Class<?>[] parameterTypes = new Class<?>[] {
				CoinReceptacleSimulator.class, Coin.class };
		Object[] args = new Object[] { this, coin };
		notifyListeners(CoinReceptacleListener.class, "coinAdded",
				parameterTypes, args);
	}
	...
	private void notifyCoinsRemoved() {
		...
		notifyListeners(CoinReceptacleListener.class, "coinsRemoved",
				parameterTypes, args);
	}
	...
	private void notifyCoinsFull() {
		Class<?>[] parameterTypes = new Class<?>[] { CoinReceptacleSimulator.class };
		Object[] args = new Object[] { this };
		notifyListeners(CoinReceptacleListener.class, "coinsFull",
				parameterTypes, args);
	}
}
