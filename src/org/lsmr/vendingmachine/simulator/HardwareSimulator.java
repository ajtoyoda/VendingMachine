...
public class HardwareSimulator {
	...
	private SelectPopButtonSimulator[] buttons;
	...
	public HardwareSimulator(int[] coinValues, int[] popCosts, String[] popNames) {
		...
		simplePopNamesArray = popNames;
		display = new DisplaySimulator();
		coinCosts = coinValues;
		...
		buttons = new SelectPopButtonSimulator[popNames.length];
		for (int i = 0; i < popNames.length; i++)
			buttons[i] = new SelectPopButtonSimulator(popNames[i], popCosts[i]);
	 ...
	 */
	public SelectPopButtonSimulator getSelectionButton(int index) {
	 ...
	 */
	/*
	 * public CardSlotSimulator getCardSlot() { return cardSlot; }
	 * 
	...
	//This function makes it easy to set up the GUI quickly
	public String getPopNameAt(int i){
		return simplePopNamesArray[i];
	}
}
